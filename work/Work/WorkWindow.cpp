...
namespace My
{
        ...
        openGLWidget(new OpenGLWidget(this))  // 创建OpenGL窗口
    {
...

        {
            ...
            // 创建并设置“绘制模式”按钮
            QPushButton* toggleDrawModeButton = new QPushButton(L("绘制模式"));
            toggleDrawModeButton->setCheckable(true);  // 按钮可选中
            toggleDrawModeButton->setChecked(false);  // 初始状态为未选中


            // 创建一个工具栏并将按钮添加到其中
            QToolBar* toolBar = addToolBar(L("状态栏"));
            toolBar->addWidget(orthoButton);
            toolBar->addWidget(toggleDrawModeButton);

            // 连接按钮点击事件
            connect(orthoButton, &QPushButton::clicked, openGLWidget, &OpenGLWidget::setIsOrtho);
            connect(toggleDrawModeButton, &QPushButton::clicked, openGLWidget, &OpenGLWidget::setIsOrtho);

            // 通过菜单打开工具栏
            QAction* showToolBarAction = new QAction(L("显示状态栏"), this);
